    ...
    <div className='container'>
      ...
      <div className='creation'>
        ...
        {orders.filter((e) => e.data.creationDate && !e.data.preparationDate && !e.data.readyDate && !e.data.deliveryDate).map((order) =>
          <OrderCard key ={ order } {...order.data} buttonText='Preparar' onClick={() => { onClick(order) }}/>
        )}
      </div>
      <div className='preparation'>
        <h1>Preparando</h1>
        {orders.filter((e) => e.data.creationDate && e.data.preparationDate && !e.data.readyDate && !e.data.deliveryDate).map((order) =>
          <OrderCard key={order} {...order.data} buttonText='Pronto' onClick={() => { onClick(order) }}
          />
        )}
      </div>
      <div className='ready'>
        <h1>Prontos</h1>
        {orders.filter((e) => e.data.creationDate && e.data.preparationDate && e.data.readyDate && !e.data.deliveryDate).map((order) =>
          <OrderCard key ={order} {...order.data} buttonText='Entregue' onClick={() => { onClick(order) }}/>
        )}
      </div>
      <div className='delivery'>
        <h1>Entregue</h1>
        {orders.filter((e) => e.data.creationDate && e.data.preparationDate && e.data.readyDate && e.data.deliveryDate).map((order) =>
          <OrderCard key={order} {...order.data} />
        )}
      </div>
    </div>
  )
